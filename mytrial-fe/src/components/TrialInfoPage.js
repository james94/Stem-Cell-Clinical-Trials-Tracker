...
import {
  TRIALS_BY_RESEARCHER_END_POINT,
  RESEARCHER_NAMES_BY_TRAIL,
} from "../settings";
...
export const TrialInfoPage = () => {
  ...
  return (
    <div>
      {trial ? (
        <TrialInfoComponent trial={trial} researchers={researchers} />
      ) : null}
      {localStorage.getItem("role") === "researcher" ? (
        <Button as={Link} to={`/patient?trial_id=${params.id}`}>
          Patient List
        </Button>
      ) : null}
    </div>
...
const TrialInfoComponent = (props) => {
  return (
    <div style={{ margin: "0px auto 20px 10px" }}>
      ...
      <p>
        Researchers:{" "}
        {props.researchers.map((e) => (
          <span key={e} style={{ marginLeft: "5px" }}>
            {e}
          </span>
        ))}
      </p>

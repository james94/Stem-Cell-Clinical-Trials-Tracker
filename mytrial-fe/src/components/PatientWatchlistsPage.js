...
import {
  PWATCHLISTS_API_BASE_URL,
  ADD_P_WATCHLIST_END_POINT,
} from "../settings";
    ...
    <Form>
      <Form.Field>
        ...
        <input
          placeholder="name"
          onChange={(e) => {
            props.setWatchlistName(e.target.value);
          }}
        />
      </Form.Field>
      <Form.Field>
        ...
        <Dropdown
          ...
          value={props.disease}
          onChange={(_, obj) => {
            props.setDiseaseInForm(obj.value);
          }}
        />
      </Form.Field>
    </Form>
...
class PatientWatchlistsPage extends React.Component {
  ...
  retrievePWatchlist = (url) => {
    fetchMyPWatchlist(url, null, { method: "GET" }, null, {
  ...
  addWatchlist = (watchlistName, disease) => {
    if (watchlistName !== null && disease !== null) {
      fetchMytrial(
        ...
        null,
        { method: "POST" },
        { name: watchlistName, diseaseArea: disease },
        { id: localStorage.getItem("id") }
      )
        .then((d) => {
          this.setOpen(false);
  ...
  render() {
    return (
      <div>
        ...
        <ul>
          {this.state.watchlists.map((e, i) => (
            <li key={i}>
              <Link
                to={{ pathname: "/trials", state: { disease: e.diseaseArea } }}
              >
                {e.name}
              </Link>
            </li>
        ...
        <Modal
          ...
          trigger={
            <Button style={{ marginLeft: "10px" }}>Add Watchlist</Button>
          }
        >
          ...
          <Modal.Content>
            <Modal.Description>
              <WatchlistForm
                name={this.state.watchListName}
                disease={this.state.diseaseInForm}
                setWatchlistName={this.setWatchlistName}
                setDiseaseInForm={this.setDiseaseInForm}
              />
            </Modal.Description>
          </Modal.Content>
          <Modal.Actions>
